...
pub struct AppSettings {
    ...
    /// Windsurf executable (or .app) path, auto-detected when possible.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// 当前 Windsurf 账号 ID（本地存储，优先于数据库 is_current）
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub fn get_windsurf_app_path() -> Option<PathBuf> {
    let settings = settings_store().read().ok()?;
    settings
        .windsurf_app_path
        .as_ref()
...
pub fn get_windsurf_user_data_dir() -> Option<PathBuf> {
    let settings = settings_store().read().ok()?;
    settings
        .windsurf_user_data_dir
        .as_ref()
...
pub fn set_windsurf_app_path(path: Option<&str>) -> Result<(), AppError> {
    let path = path
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(ToString::to_string);
    mutate_settings(|settings| settings.windsurf_app_path = path.clone())
}

pub fn set_windsurf_user_data_dir(path: Option<&str>) -> Result<(), AppError> {
    let path = path
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(ToString::to_string);
